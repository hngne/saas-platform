...
export class CategoryController {
  ...
  search = async (req: Request, res: Response) => {
    const keyword = (req.query.keyword || req.query.q) as string;
    if (!keyword) {
  ...
  create = async (req: Request, res: Response) => {
    // Nếu có file upload → lấy Cloudinary URL
    const file = req.file as MulterFile | undefined;
    if (file?.path) {
      req.body.image_url = file.path;
    }
    // Parse số nếu gửi qua multipart
    ...
    await cache.delPattern(`categories:${req.user!.tenantId!}:*`);
